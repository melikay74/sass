...
function _updateComments(){var comments,jsonUrl;isLoading=true;_setCurrentCommentsContent(loadingIcon);_setCommentOffset();jsonUrl=["http://www.kcet.org/cgi-bin/mt/mt-comments.cgi?__mode=comment_listing&direction=",direction,"&entry_id=",entryID,"&limit=",commentsPerPage,"&offset=",_getCommentOffset()].join('');if(!commentAnchor){commentAnchor="comments-content";}
...
function _refreshComments(commentData){_setCommentContent(commentData);_setCurrentComments();window.location.hash='reset';window.location.hash=commentAnchor;_toggleNavLinks();}})();function mtScore(entry_id){var span=DOM.getElement('scoring-id-'+entry_id);if(!span)return false;if(DOM.hasClassName(span,'scoring-pending'))return false;var u=mtGetUser();var p='&ow=1';if(DOM.hasClassName(span,'scoring-scored')){if(u&&u.is_authenticated){DOM.removeClassName(span,'scoring-scored');p+='&score=0';}else{return false;}}
if(!DOM.hasClassName(span,'scoring-scorable'))return false;DOM.addClassName(span,'scoring-pending');var sid=u?u.sid:0;var script=document.createElement('script');script.src='http://www.kcet.org/cgi-bin/mt/mt-cp.cgi?__mode=vote&blog_id=92&f=scored,count&jsonp=mtScore_cb&id='+entry_id+'&magic_token='+sid+p;(document.getElementsByTagName('head'))[0].appendChild(script);return false;}
...
if(entry_ids=='')return false;var xh=mtGetXmlHttp();if(!xh)return false;var script=document.createElement('script');script.src='http://www.kcet.org/cgi-bin/mt/mt-cp.cgi?__mode=score&blog_id=92&f=scored,count&jsonp=mtScore_cb&id='+entry_ids;(document.getElementsByTagName('head'))[0].appendChild(script);return false;}
...
function script_follow(id){var u=mtGetUser();if(!u||!u.name)return;var xh=mtGetXmlHttp();if(!xh)return false;xh.open('POST','http://www.kcet.org/cgi-bin/mt/mt-cp.cgi',true);xh.onreadystatechange=function(){if(xh.readyState==4){if(xh.status&&(xh.status!=200)){}else{eval(xh.responseText);}}};xh.setRequestHeader('Content-Type','application/x-www-form-urlencoded');xh.send('__mode=follow&id='+id+'&magic_token='+u.sid+'&jsonp=follow');DOM.addClassName('following_'+id+'_else','hidden');var span=document.getElementById('following-status');if(span)span.innerHTML='<img src="http://ops.kcet.org/mt-static/images/indicator.white.gif" height="10" width="10" alt="Following..." />';}
function script_leave(id){var u=mtGetUser();if(!u||!u.name)return;var xh=mtGetXmlHttp();if(!xh)return false;xh.open('POST','http://www.kcet.org/cgi-bin/mt/mt-cp.cgi',true);xh.onreadystatechange=function(){if(xh.readyState==4){if(xh.status&&(xh.status!=200)){}else{eval(xh.responseText);}}};xh.setRequestHeader('Content-Type','application/x-www-form-urlencoded');xh.send('__mode=leave&id='+id+'&magic_token='+u.sid+'&jsonp=leave');DOM.addClassName('following_'+id,"hidden");var span=document.getElementById('following-status');if(span)span.innerHTML='<img src="http://ops.kcet.org/mt-static/images/indicator.white.gif" height="10" width="10" alt="Leaving..." />';}
...
var mtCookieName="mt_blog_user";var mtCookieDomain=".kcet.org";var mtCookiePath="/";var mtCookieTimeout=14400;function mtHide(id){var el=(typeof id=="string")?document.getElementById(id):id;if(el)el.style.display='none';}
...
return user;}
var mtFetchedUser=false;function mtFetchUser(cb){if(!cb)cb='mtSetUser';if((cb=='mtSetUser')&&mtGetUser()){var url=document.URL;url=url.replace(/#.+$/,'');url+='#comments-open';location.href=url;}else{mtFetchedUser=true;var u=mtGetUser();var script=document.createElement('script');var ts=new Date().getTime();script.src='http://www.kcet.org/cgi-bin/mt/mt-comments.cgi?__mode=userinfo&blog_id=92&jsonp='+cb+'&ts='+ts+'&sid='+u.sid;(document.getElementsByTagName('head'))[0].appendChild(script);}}
function mtVerifySession(cb){if(!cb)return;var script=document.createElement('script');var ts=new Date().getTime();var u=mtGetUser();script.src='http://www.kcet.org/cgi-bin/mt/mt-comments.cgi?__mode=verify_session&blog_id=92&jsonp='+cb+'&ts='+ts+'&sid='+u.sid;(document.getElementsByTagName('head'))[0].appendChild(script);}
...
f.armor.value='ea4e28c090e7548b4ee8178bd6abaf533ce6a276';if(f.bakecookie&&f.bakecookie.checked)
...
return true;}
return false;}
function mtCommentSessionVerify(app_user){var f=document['comments_form'];if(app_user&&app_user.verified){f.submit();}else{alert('Your session has expired. Please sign in again to comment.');mtClearUser();mtFireEvent('usersignin');mtShow('comments-form');mtHide('comments-open-footer');}}
...
if(el)
el.innerHTML='Signing in... <span class="status-indicator">&nbsp;</span>';mtClearUser();mtSignIn();return false;}
function mtSetUserOrLogin(u){if(u&&u.is_authenticated){mtSetUser(u);}else{mtSignIn();}}
function mtSignOut(entry_id){mtClearUser();var doc_url=document.URL;doc_url=doc_url.replace(/#.+/,'');var url='http://www.kcet.org/cgi-bin/mt/mt-cp.cgi?__mode=logout&blog_id=92';if(is_preview){if(document['comments_form']){var entry_id=document['comments_form'].entry_id.value;url+='&entry_id='+entry_id;}else{url+='&return_url=http%3A%2F%2Fwww.kcet.org%2F';}}else{url+='&return_url='+encodeURIComponent(doc_url);}
location.href=url;}
...
cookieEndIndex=c.length;return decodeURIComponent(c.substring(cookieStartIndex+prefix.length,cookieEndIndex));}
...
window.onload=mtInit;function mtLoggedIn(ott){var script=document.createElement('script');var ts=new Date().getTime();script.src='http://www.kcet.org/cgi-bin/mt/mt-comments.cgi?__mode=userinfo&blog_id=92&jsonp=mtSaveUserInfo&ott='+ott;(document.getElementsByTagName('head'))[0].appendChild(script);}
function mtRefreshUserInfo(sid){var script=document.createElement('script');var ts=new Date().getTime();script.src='http://www.kcet.org/cgi-bin/mt/mt-comments.cgi?__mode=userinfo&blog_id=92&jsonp=mtSaveUserInfo&sid='+sid;(document.getElementsByTagName('head'))[0].appendChild(script);}
function mtSaveUserInfo(u){if(u.error){if(!user){alert('The sign-in attempt was not successful; please try again.');}
return;}
...
else if(command.match(/^login_(.*)$/)){var sid=RegExp.$1;var u=mtGetUser();if(u&&u.sid==sid){mtFireEvent('usersignin');}
else{mtLoggedIn(sid);}}
